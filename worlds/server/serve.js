...
app.get('/position', (req, res) => {
    for (let robot of robot_positions) {
        if (robot.name === 'EPUCK1') {
            robot.position = [req.query.r1_x, req.query.r1_y, req.query.r1_angle];
        } else if (robot.name === 'EPUCK2') {
            robot.position = [req.query.r2_x, req.query.r2_y, req.query.r2_angle];
        } else if (robot.name === 'EPUCK3') {
            robot.position = [req.query.r3_x, req.query.r3_y, req.query.r3_angle];
        }
    }
    res.json({ response: 'received' });
});
app.get('/target', (req, res) => {
    ...
    for (let robot of robot_positions) {
        ...
        minDist = robot.pathLength < minDist ? robot.pathLength : minDist
        console.log(robot.name, robot.pathLength);

    }
    minRobot = robot_positions.find(_ => _.pathLength == minDist)
    // You may want to send back a more informative response
...
function convertCoordinate(inputValue) {
    ...
    let convertedValue = Math.floor(normalizedValue * 7 / 1.75);
...
function exploreMaze(maze, start, goal) {
    ...
    mazeStart = maze[convertCoordinate(start[0])][convertCoordinate(start[0])]
...
let maze = initializeMaze(8, 8, walls);
